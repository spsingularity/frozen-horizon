...
\keywords{Inflation, $f(R)$ gravity, Primordial power spectrum, Cosmic microwave background, Reheating}
\date{\today}
...
scalaron mode, so that inflation begins at a regular Euclidean four-sphere,
not a singularity. The construction is fixed by a zero-work
condition on the trace-flow wall in a declared integration measure and by a
unit spectral gap $\mu^2=d-1$; integrality of $p$ follows from analyticity
of the action at $R=0$. Two walls are analyzed, the canonical invariant
measure and the coordinate rule, paired with conformal and minimal Higgs
couplings. The
model retains the Starobinsky predictions $n_s\simeq0.962$--$0.965$ and
$r\simeq0.004$, and adds an approximately $50\%$ localized deficit in the
scalar power spectrum near $k\sim10^{-4}\,\mathrm{Mpc}^{-1}$, with no tensor
counterpart. The deficit location is exactly degenerate between $p$ and the
reheating history, but the latter is calculable because the low-curvature
limit is exactly $R+R^2/6M^2$: the coupled scalaron--radiation system yields
$N_*=50.6$--$51.6$, set by the Higgs curvature coupling, with an uncertainty
of $0.24$ e-folds against an integer spacing of $0.94$--$0.99$, closing the
degeneracy so that each integer predicts a definite feature location.
Propagating the exact spectra through CAMB and evaluating the official
Planck 2018 low-$\ell$ Commander and SimAll likelihoods, we find one
integer of each branch excluded
($\Delta\chi^2=+13.6$ and $+8.5$), the preferred integers giving
$\Delta\chi^2_{TT+EE}=-1.5$ to $-1.7$, and evidence marginalized over the
ladder of $+1.0$, consistent with a statistical fluctuation. The deficit lies
below cosmic variance and cannot be confirmed by the low-$\ell$ CMB.
\end{abstract}
...
$\sigma(N_*)\simeq0.24$ e-folds against an integer spacing of $0.94$--$0.99$
e-folds,
...
integer of each model branch ($\Delta\chi^2=+13.6$ and $+8.5$), while the
preferred integers improve the fit by $-1.5$ to $-1.7$, with a marginalized
...
\caption{Trace-flow wall $S(z)=(1-z)(1+\alpha z)$ for three of the four
integration measures tabulated in Sec.~\ref{sec:measure} and, for
contrast, the shoulderless linear wall $\alpha=0$. The quantity $S$ is the
...
scalar amplitude, the construction thus has three free inputs: the integer
$p$, the zero-work measure of Sec.~\ref{sec:measure}, and the matter
boundary condition $\xi_H$ of Sec.~\ref{sec:relabeling}. All
...
inflationary action. Numerically $dN_f/dp=0.993$ for the coordinate wall and $0.946$ for the
...
For a feature family the statement is exact: the feature wavenumber and
the reheating history form a flat direction, which no fit can break and
only a computed reheating history can close.
...
The sudden-decay approximation is not used. The coupled
...
The sudden-decay shortcut differs by $0.20$ e-folds---one fifth of the rung
...
stated at $k=0.002\Mpc$ and converts to $51.2$ at $k_*=0.05\Mpc$;
...
and sudden-decay treatments, $0.20$; the measured equation-of-state
deviation, $0.05$; and $g_*$, $0.01$---in quadrature
$\sigma(N_*)\simeq0.24$ e-folds against a rung spacing of $0.94$--$0.99$. The
integers are therefore separated by approximately four standard deviations
($3.98$ at the closest spacing, $4.19$ at the widest), and
...
deficit by $e^{0.95}$ in $k$; the far-infrared rise at the left edge, which
...
phenomenology requires $\alpha>1$---at $\alpha\le1$ the maximum of $S$ sits at
$z=0$ with $S=1$---and hence both high-curvature operators;
...
at $\ell\le30$ to better than $1.1\times10^{-3}$ in $TT$, $EE$ and $TE$
(and better than $6.7\times10^{-4}$ in $TT$ and $EE$). Table~\ref{tab:cmbratios}
...
broad and narrow patterns two integers lower, and its likelihood values are
...
imprint: for the broad pattern the $\ell=2$ ratios are $0.68$ in $EE$ against
$0.84$ in $TT$ (Table~\ref{tab:cmbratios}). This is the model-level counterpart of the long-standing
...
not by the quadrupole (both $\simeq16\%$ suppressed) but by the
...
signature is the only internal discriminant within the family; both members
...
polarization penalty of the broad pattern by more than an order of
magnitude ($+1.33$, against $+0.07$ from the exact non-Gaussian SimAll
likelihood); all $EE$
contributions under the exact likelihood are below $0.75$ in magnitude, while
the temperature differences survive essentially unchanged.
\input{tables/likelihood.tex}
...
$\Delta\chi^2=+13.6$ (coordinate branch, $p{=}62$) and $+8.5$ (invariant
...
parameter, $-1.5$ to $-1.7$ with no fitted feature parameter is
comparable. The marginalized evidence of Table~\ref{tab:evidence} is the
Bayesian counterpart, and carries the look-elsewhere penalty
explicitly: $+1.70$ for the best single integer against $+0.99$
marginalized. This addresses the a-posteriori-selection critique of
low-multipole explanations \cite{Bennett2011,Planck2018VII}.
The exclusions have no analogue in continuous-cutoff fits: because the
members of the family carry no adjustable scale, individual members can
be rejected, whereas a continuous cutoff can fail to be rewarded by the
data without being excluded by it. A suppression at
$k\simeq2$--$3\times10^{-4}\Mpc$ has been shown to move the large-angle
correlation statistic $S_{1/2}$ toward its observed low value
\cite{Melia2021,Copi2015}; the broad member of each branch, though not
the preferred one, lies in that range.
...
causal-patch measure of the exit epoch, often listed among the assumptions
of such constructions, is not an independent input here: every observable
is referenced to the end of inflation on a strongly
...
$n_s\simeq0.962$--$0.965$, $r\simeq0.004$, a scalar deficit of depth
$\sim0.5$ at a computed wavenumber, no tensor counterpart above
$2\times10^{-3}$, no isocurvature, and the correlated
...
family, and lies some one hundred and ten orders of magnitude below the present
...
The principal open problem is ultraviolet, and it is well posed. Both
...
outcome would settle the question: a derivation would convert the feature
location into a
...
$R=0$, so the family of completions is discrete rather than continuous.
Three inputs remain free: the value of $p$, the integration measure
entering the zero-work condition, and the matter boundary condition
$\xi_H$, which relabels the ladder rather than moving it.
...
$N_*=50.6$--$51.6$ with an uncertainty of $0.24$ e-folds against an integer
...
each branch is excluded ($\Delta\chi^2=+13.6$ and $+8.5$), while the
