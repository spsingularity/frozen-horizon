...
% REVTeX 4.2, Physical Review D two-column format.
...
\thanks{ORCID: \href{https://orcid.org/0009-0005-8153-071X}{0009-0005-8153-071X}}
\affiliation{Independent researcher, Santa Clara, California 95050, USA}

\keywords{Quantum cosmology, $f(R)$ gravity, No-boundary proposal, Picard--Lefschetz theory, Inflation}
\date{\today}

\begin{abstract}
Consider a metric $f(R)$ theory with an exact constant-curvature solution at
which the scalaron is tachyonic, with mass $m_H^2=-\mu^2H_H^2$. Its Euclidean
continuation is a compact four-sphere, a candidate origin for the
inflationary history, and the saddle is admissible as a decay channel when it
carries exactly one negative mode, which requires $0<\mu^2<4$. This condition
is shown to be necessary but not sufficient. Continuing the unstable
direction through the equator and demanding that the growing Lorentzian
branch be real selects a unique complex contour, rotated by an angle
$\theta$, along which the Born measure is normalizable only when
$\cos2\theta<0$. Both mode equations are hypergeometric, and the connection
coefficients reduce through the Gamma reflection formulae to the closed form
$\theta=\pi s/2$, with $s=\tfrac12[\sqrt{9+4\mu^2}-3]$ the growth exponent.
Normalizability is therefore exactly $s>1/2$, so admissible tachyonic
horizons satisfy $7/4<\mu^2<4$. The boundary fluctuation kernel is likewise
obtained in closed form for every harmonic, which establishes the
one-negative-mode structure for the entire tower rather than for a tabulated
subset. The state is developed in detail at the unit spectral gap
$\mu^2=d-1=3$ in $d=4$ dimensions, where the classicalizing contour lies at
$\theta=\tfrac{\pi}{4}(\sqrt{21}-3)=71.216^\circ$, and the background lapse
integral is performed exactly under a Neumann regularity condition. Because
the spatial sections are $S^3$, the early noise is not a continuum: harmonic
$n$ enters the long-wavelength sector at $N_n=\ln(n+1)$ exactly, and the exit
occurs before a continuum diffusion coefficient is established. The
discreteness is a property of the initial state rather than of the observable
sky.
\end{abstract}
...
perturbations \cite{FLT2017,FLT2017b}---a reading contested in
\cite{DDHHJ2017}---while alternative boundary conditions restore a
well-behaved state \cite{DiTucciLehners2019,DiTucci2019,Lehners2023}. Any claim that
...
mode \cite{Coleman1988,ColemanDeLuccia1980}, which bounds $0<\mu^2<4$.
Our principal
...
$\sqrt{n/2}$ to leading order, and carrying the next order gives
\begin{equation}
...
One numerical caution applies to Eq.~(\ref{eq:euclid}). Imposing the
regular behaviour literally as $f_n(u_0)=u_0^{\,n}$ starts the
integration far beneath the absolute tolerance of a standard adaptive
solver: with $u_0=10^{-6}$ and $\mathrm{atol}=10^{-14}$ the initial
amplitude falls below the error control from $n=3$, and by $n=8$ it is
$10^{-48}$. The integration then returns numerical noise rather than the
regular mode, without any indication of failure; with the solver used
here the literal start reproduces the exact ratio through $n=7$ and
breaks down at $n=8$. All results here normalize $f_n(u_0)=1$
...
which fails in the shaded band, that is for $\mu^2<7/4$---the value at
which the growth exponent reaches $s=1/2$. The unit spectral gap $\mu^2=3$ lies inside
...
about the horizon, the two forces are equal at
$\delta\phi_\times=2\mu^2H_H^2/|W_3|$, and the ratio at any smaller
amplitude is $\delta\phi/\delta\phi_\times$; self-consistency requires
$\delta\phi_\times$ to lie far into the tail of the Born measure of
Eq.~(\ref{eq:widths}). Whether it does is a property of the particular
curvature function. For the worked example of the companion paper
$W_3=-7.7\times10^{-10}\Mpl$, so the ratio
of cubic to linear force is $1.1\times10^{-5}$ at the drift threshold, and the
...
initial size \cite{DiTucciLehners2019,DiTucci2019}. Regular compact
closure then requires the
Neumann condition
\begin{equation}
...
number. This selection reproduces, for the frozen-horizon background,
the result established for Robin conditions in
\cite{DiTucciLehners2019}: the unstable saddle associated with the
zero-size Dirichlet prescription is eliminated and the Hartle--Hawking
pair survives. The decomposition assumes the thimbles may be traversed
in full. Under the Kontsevich--Segal allowability criterion for complex
metrics \cite{Witten2021}, the de Sitter saddles lie at the edge of the
allowable domain and the lapse thimbles are cut off where they leave it
\cite{Lehners2022}; whether that truncation alters the intersection
numbers here is not established, and we return to it in
Sec.~\ref{sec:discussion}. What the present calculation adds is that the background
contour is settled exactly for this saddle, rather than inferred from a
local Hessian, and that the scalar sector can be carried along with it,
as the remainder of this section shows.
...
exit width is suppressed by $(\sigma_{C_g}/\Mpl)^2\simeq3\times10^{-12}$ in
...
factor of three, within the epoch in which the exit occurs.}
\label{fig:shells}
\end{figure}
...
\label{sec:exit}
Exit is treated by the first-passage methods standard in stochastic
inflation \cite{VenninStarobinsky2015}, in which the moments of the
e-fold count follow from the time taken to reach a threshold. The
coarse-grained unstable mode $X=\phi_H-\phi$ obeys
...
during this epoch is discrete: the variance is injected in the finite
steps $\Delta V_n$ of Eq.~(\ref{eq:shellvar}), at the times
$N_n=\ln(n+1)$, and the first inhomogeneous shell does not cross until
$\ln2=0.693$ e-folds. Since the mean waiting time above is $0.843$
e-folds, the exit occurs while only the first one or two shells have
contributed, and the continuum diffusion coefficient is not yet
established. A first-passage treatment built on the discrete kernel of
Eq.~(\ref{eq:noisekernel}) rather than on white noise is therefore
required for a quantitative exit-time distribution; we do not construct
one here, and the moments quoted above are continuum values retained as
a benchmark.

Both results remain operational rather than predictive. The threshold
$X_c$ is inherited from a continuum
...
companion paper gives a total duration of $70.35\pm0.82$ e-folds, a
benchmark rather than a prediction.

\section{Observability of the staircase}
\label{sec:observability}
Whether the discrete spectrum of crossing times survives into the
observable sky determines its phenomenological relevance. The results
below indicate that it does not.
...
\label{eq:kmap}
\end{equation}
where $\chi_*=13870\,$Mpc is the comoving distance to last scattering,
$N_*$ is the pivot's e-fold distance to the end of inflation and
...
\end{aligned}
\end{equation}
which is smaller than the present horizon scale by more than five orders
of magnitude. At the wall-induced feature of the companion paper,
$k\simeq9.32\times10^{-5}\Mpc$, the corresponding shell index and spacing are
\begin{equation}
n\simeq7\times10^{5},
\qquad
...
The result delimits the reach of the analysis. The discreteness
established in
...
horizon within a well-specified domain. Its boundaries are the
following.

Four limitations are structural rather than technical. First, the
...
$\epsilon$, which at present is mild but not absent. Fourth, the contour
selected here is fixed by normalizability and by reality of the growing
branch, criteria internal to the quadratic theory. The
Kontsevich--Segal criterion \cite{Witten2021} restricts admissible complex
metrics from an independent direction, and has been shown to cut off the
lapse thimbles in minisuperspace \cite{Lehners2022} and to exclude
inflationary saddles through the tachyonic instabilities of their
perturbation spectra \cite{HertogJanssenKarlsson2023}. Whether the
classicalizing contour of Sec.~\ref{sec:thimble} is allowable in that
sense, and whether the two criteria agree on the window, is not addressed
here and would sharpen the result either way.
...
sole negative direction and every inhomogeneous scalar harmonic Gaussian
suppressed---therefore holds for the whole tower and not merely for the
harmonics tabulated. The physical tensor harmonics are suppressed
separately, by the bulk Hessian of Sec.~\ref{sec:kernel}. The kernel grows as
...
shell injects three times the continuum variance. Since the mean waiting
time is $0.843$ e-folds, the exit occurs while only the first one or two
shells have contributed. The findings indicate that continuum stochastic
treatments are not reliable in the epoch immediately following a
finite-radius waist.
