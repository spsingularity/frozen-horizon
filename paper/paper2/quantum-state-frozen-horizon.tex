...
in closed form. Let $s$ denote the late-time growth exponent, the positive
root of
\begin{equation}
s(s+3)=\mu^2,
\qquad
...
\label{eq:sdef}
\end{equation}
so that the two Lorentzian exponents of Eq.~(\ref{eq:lorentz}) are $s$ and
$-(3+s)$. Both mode equations are hypergeometric. Substituting
...
\label{eq:cratio}
\end{equation}
The Euclidean equation is the same equation at $x=\cos u$, namely the
Gegenbauer equation $(1-x^2)f''-4xf'+\mu^2f=0$ of index $3/2$ and degree $s$.
Its indicial exponents at the pole are $0$ and $-2$; requiring the regular
one, that is demanding that the coefficient of the singular
$(1-x^2)^{-1}$ branch vanish, fixes the equatorial logarithmic derivative to
\begin{equation}
r_E=2\,
...
\label{eq:rE}
\end{equation}
which is negative for $0<s<1$. In the product $|r_E|\,c_2/c_1$ every Gamma
